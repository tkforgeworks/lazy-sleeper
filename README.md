...
**Current release: [v0.1.2](CHANGELOG.md) (2026-08-28)** — the code to draft on 2026-09-04. Releases
are tags + changelog notes; the deliverable is this repo at the tag plus the Supabase schema
(Alembic head `0014`) and a `.env`.

- **M0–M2 done (2026-08-19):** ingestion + immutable snapshot archive, league scoring engine (QB–DEF,
  parity vs nflverse), season + weekly provider benchmarks, provider abstraction + fitted ensemble
  weights, and the ops floor — shared Supabase Postgres, archive mirrored to Supabase Storage, daily
  pull from GitHub Actions.
- **M3 done (2026-08-20):** replacement baselines, flex-aware VORP, tiers + cliff flags, ADP-delta +
  provider-disagreement flags, persisted daily boards at `GET /board` / `/board.html`.
- **M4 done — v0.1.0 (2026-08-26):** the live draft companion: pick poller, draft state, signals,
  recompute engine, `GET /draft/{id}/state` + the self-contained fallback page, `lazy serve`, three
  recorded mock drafts replayed in CI.
- **v0.1.1 (2026-08-28):** the six live-path bugs from the 0.1.0 review (DB-outage resilience,
  undo/repick, runner death, backoff cap, page redraw rules, mobile).
- **v0.1.2 (2026-08-28):** DB pool timeouts/keepalives (no more 15-minute hangs), 404-safe runner +
  clean shutdown, and the Flutter contract additions — pick clock, on-the-clock team, recent-pick feed,
  bye weeks. The API contract for the app is `docs/api/` (`openapi.json` + generated `README.md` +
  hand-written `GUIDE.md`), regenerated by `lazy api export` and drift-checked by a test.

Next: 0.2.0 in-season (M7) — LS-51 freshness, ForgeModel (M5, LS-40–43), Tailscale. See
