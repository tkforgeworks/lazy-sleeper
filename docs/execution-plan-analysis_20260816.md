...
| E2 Ingestion & snapshot archive | LS-2 | LS-14 projections/ADP → core (shipped as `core.stat_lines(category)` + `core.adp`) · LS-15 nflverse actuals/snaps/kicking/xFP → core (actuals also go into `core.stat_lines`) · LS-16 draft picks/rosters → core · LS-17 daily pull scheduler |
